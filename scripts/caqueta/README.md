...
5. **Case outlines, dips, and categories.** Each case is outlined from a Sentinel-2 NBR drop around a RADD seed. Coherence dips are measured against the case's **own recent history**: the weather-corrected (forest-subtracted) 80 m coherence of a pair minus the median of the previous three pairs. A dip must exceed 2.5σ, where σ is the same quantity for intact-forest areas of the case's size. At 2.5σ, about 5% of intact-forest areas show any false dip over the series (19–25% at 2σ, 1–2% at 3σ). Measuring against the case's own history, not intact forest, also catches burns of already-cleared land, which fall from well above forest coherence to about forest level. Cases are then categorized by land state before the event (Sentinel-2 NBR) and by whether a dip occurred.
